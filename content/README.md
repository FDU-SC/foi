...
content](../README.md#换掉-content)」：CI 每个 PR 把这个目录里的实质性内容全部
删掉（只留顶部八份 `*-modules.ts`）再跑一遍类型检查、lint、构建与冒烟，另有一条
用例断言内核源码里不出现这里的任何一个名字。所以下面写到的任何东西——包括
`traditional` 这个后端名、`demo-acm` 这场比赛、「省选」这个难度——内核都不认识。

八份 `*-modules.ts` 就在这一层。它们是内核声明 server/client 边界的地方，扫的
是 `./problems/...`、`./contests/...` 这些实质路径，换掉或抽空下面的内容不换它们。
...
上面这些几乎全是可选的。严格说一样都不需要——只留顶部八份 `*-modules.ts`，平台
照样起得来，八个注册表都把「空」当作合法部署，只是没有东西可做。要让它有用，最
少是一道题：一份 `problems/<slug>/problem.ts` 加一份 `statement.mdx`；题目挂在外
部后端上的话，再加 `backends.ts` 里的一条名册。
