import { describe, expect, it } from "vitest";
import { fail, input, participants, problem, solve } from "@/test/standings-support";
import type { CtfCell } from "./ctf-dynamic";
import { ruleset as ctfDynamicRuleset } from "./ctf-dynamic";

function compute(options: Parameters<typeof input>[0]) {
  return ctfDynamicRuleset.compute(input(options));
}

function cell(
  standings: ReturnType<typeof compute>,
  uid: number,
  slug: string,
): CtfCell | undefined {
  const row = standings.rows.find((entry) => entry.participant.uid === uid);
  return row?.cells[slug] as CtfCell | undefined;
}

const problems = [problem("a", "A"), problem("b", "B")];

const plain = { bloodBonus: [] as number[] };

describe("ctf 动态分值衰减", () => {
  it("只有一个人解出时是满值", () => {
    const standings = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [solve(1, "a", 10)],
    });

    expect(cell(standings, 1, "a")?.score).toBe(500);
  });

  it("解出的人越多，题目越不值钱", () => {
    const values = [1, 2, 3, 5, 10].map((count) => {
      const uids = Array.from({ length: count }, (_, i) => i + 1);
      const standings = compute({
        participants: participants(...uids),
        problems,
        config: plain,
        submissions: uids.map((uid, i) => solve(uid, "a", 10 + i)),
      });
      return cell(standings, 1, "a")?.score ?? 0;
    });

    for (let i = 1; i < values.length; i += 1) {
      expect(values[i]).toBeLessThan(values[i - 1]);
    }
    expect(values[0]).toBe(500);
  });

  it("衰减不会跌破 minimum", () => {
    const uids = Array.from({ length: 60 }, (_, i) => i + 1);
    const standings = compute({
      participants: participants(...uids),
      problems,
      config: { ...plain, minimum: 100 },
      submissions: uids.map((uid, i) => solve(uid, "a", 10 + i)),
    });

    expect(cell(standings, 1, "a")?.score).toBe(100);
  });

  it("initial / minimum / decay 可配置", () => {
    const standings = compute({
      participants: participants(1),
      problems,
      config: { ...plain, initial: 1000 },
      submissions: [solve(1, "a", 10)],
    });

    expect(cell(standings, 1, "a")?.score).toBe(1000);
  });
});

describe("ctf 血奖", () => {
  it("前三名解出者按顺序拿到加成", () => {
    const uids = [1, 2, 3, 4];
    const standings = compute({
      participants: participants(...uids),
      problems,
      config: { bloodBonus: [0.1, 0.05, 0.02] },
      submissions: uids.map((uid, i) => solve(uid, "a", 10 + i)),
    });

    const value = cell(standings, 4, "a")?.score ?? 0;
    expect(cell(standings, 1, "a")?.score).toBeCloseTo(value * 1.1, 6);
    expect(cell(standings, 2, "a")?.score).toBeCloseTo(value * 1.05, 6);
    expect(cell(standings, 3, "a")?.score).toBeCloseTo(value * 1.02, 6);
  });

  it("血次按解出先后而非提交先后", () => {
    const standings = compute({
      participants: participants(1, 2),
      problems,
      config: { bloodBonus: [0.1, 0.05] },
      submissions: [

        fail(1, "a", 5),
        solve(2, "a", 10),
        solve(1, "a", 20),
      ],
    });

    expect(cell(standings, 2, "a")?.blood).toBe(1);
    expect(cell(standings, 1, "a")?.blood).toBe(2);
  });

  it("超出 bloodBonus 长度的名次没有血标记", () => {
    const uids = [1, 2];
    const standings = compute({
      participants: participants(...uids),
      problems,
      config: { bloodBonus: [0.1] },
      submissions: uids.map((uid, i) => solve(uid, "a", 10 + i)),
    });

    expect(cell(standings, 1, "a")?.blood).toBe(1);
    expect(cell(standings, 2, "a")?.blood).toBeNull();
  });
});

describe("ctf 失败尝试", () => {
  it("未解出的题也留下 attempts", () => {
    const standings = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [fail(1, "a", 10), fail(1, "a", 20)],
    });

    expect(cell(standings, 1, "a")).toMatchObject({
      score: 0,
      solvedAt: null,
      attempts: 2,
    });
    expect(standings.rows[0].total).toBe(0);
  });

  it("解出后的提交不再累加 attempts", () => {
    const standings = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [
        fail(1, "a", 10),
        solve(1, "a", 20),
        fail(1, "a", 30),
      ],
    });

    expect(cell(standings, 1, "a")?.attempts).toBe(2);
  });
});

describe("ctf 参赛名单", () => {

  it("非参赛者的 AC 不改变分值衰减", () => {
    const alone = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [solve(1, "a", 10)],
    });
    const withOutsider = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [solve(1, "a", 10), solve(99, "a", 20)],
    });

    expect(cell(alone, 1, "a")?.score).toBe(500);
    expect(cell(withOutsider, 1, "a")?.score).toBe(
      cell(alone, 1, "a")?.score,
    );
  });

  it("非参赛者不占血", () => {
    const standings = compute({
      participants: participants(1, 2),
      problems,
      config: { bloodBonus: [0.1, 0.05] },
      submissions: [
        solve(99, "a", 5),
        solve(1, "a", 10),
        solve(2, "a", 20),
      ],
    });

    expect(cell(standings, 1, "a")?.blood).toBe(1);
    expect(cell(standings, 2, "a")?.blood).toBe(2);
  });
});

describe("ctf 总分与排名", () => {
  it("总分是各题得分之和，tiebreak 是末次解题时刻", () => {
    const standings = compute({
      participants: participants(1),
      problems,
      config: plain,
      submissions: [solve(1, "a", 10), solve(1, "b", 30)],
    });

    expect(standings.rows[0].total).toBe(1000);
    expect(standings.rows[0].tiebreak).toBe(30 * 60_000);
  });

  it("解出更多题的人排在前面", () => {
    const standings = compute({
      participants: participants(1, 2),
      problems,
      config: plain,
      submissions: [
        solve(1, "a", 10),
        solve(2, "a", 20),
        solve(2, "b", 30),
      ],
    });

    expect(standings.rows[0].participant.uid).toBe(2);
  });

});
