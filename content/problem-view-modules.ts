 ...
 * this is its own file rather than part of `./problem-modules.ts`: that glob
 * carries answers and testdata paths, this one carries components, and
 * components render in the browser. A `views.tsx` is public by construction,
 * so nothing in one may reach for `problem.ts`.
 *
 ...
 * `problemViews`. Absent — as in a tree whose substance has been stripped —
 * every problem falls back to whatever its own `views.tsx` says, and a
 * problem with neither renders as JSON.
 */
