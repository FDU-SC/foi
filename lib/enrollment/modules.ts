 ...
 *
 * Two registries are built out of these files — `lib/auth/groups.ts` for the
 ...
 * from, not because of where they are declared. `lib/auth/groups.ts` is
...
export interface EnrollmentSource {
  ...
  path: string;
  /** Group declarations, unvalidated — see `lib/auth/groups.ts`. */
  groups?: unknown;
