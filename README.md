...
- `/problems`：左侧题单栏 + 右侧题表
  - 左栏选范围；右侧在该范围下用「题目」「排行榜」两个 tab 切换（排行榜在 `/leaderboard`）
  - 题单栏每场比赛一项，按比赛的 `domain`（方向）分组
  - 题表上方是筛选器：搜索、难度、标签、状态、排序，结果分页
- `/problems?domain=<方向>`：同一页面，列出这个方向下所有题单的题目；只有一个题单的方向不单独成列表
...
- 页面：`/leaderboard`，题库右侧的「排行榜」视图
  - 范围：与题单页同一套左栏；「全部题目」、各方向的「全部」、方向下的分区（`board` / `section`）
  - 时间：`from` / `to`，在一个日历里选起止日期，按站点时区计，含结束当天；不选就是全部时间
  - 表格上方按昵称或用户名搜索，结果保留全榜名次；每页 50 人；顶部显示自己的排名
- 题库只有 `/leaderboard` 这一个排行榜页面，旧地址 `/problems/<分区>/standings` 跳转到这里
