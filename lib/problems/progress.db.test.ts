...
describeDb("个人进度读取", () => {
  ...
  it("可读所有人的用户仍只聚合自己的当前比赛完整历史", async () => {
    ...
    try {
      ...
      expect(result.get(pair.problem.slug)?.state).toBe("solved");
      ...
      expect(second.get(pair.problem.slug)?.state).toBe("attempted");
  ...
  it("游客无进度，未配置解释的题目不返回进度", async () => {
    ...
    try {
      ...
      expect((await progressFor(pair.contest.slug, viewerWith("submission.read", uids[0]))).has(pair.problem.slug)).toBe(false);
  ...
  it("重判清除结果后重新计算", async () => {
    ...
    expect((await progressFor(pair.contest.slug, viewerWith("submission.read", uids[0]))).get(pair.problem.slug)?.state).toBe("attempted");
  });
});
